 ...
 */
export const forMatNext = function(
  to,
  Intercept,
  next,
  { selfRoutes, CONFIG }
  ...
  return new Promise(async resolve => {
    if (Reflect.get(Router, "H5RouterReady")) {
      const res = await new Promise(async resolve => {
        let { toRoute, fromRoute } = getRouterNextInfo(to, from, Router);
  ...
  return new Promise(async resolve => {
    ...
    vuelifeHooks.beforeHooks[0](to, from, async Intercept => {
      ...
      const res = await new Promise(async resolve => {
        ...
        // 修复beforeHooks 只触发第一个的问题
        async function exec(i) {
          if (i >= lifeCycle["beforeHooks"].length) {
            return resolve(result);
          }
          await lifeCycle["beforeHooks"][i](
            toRoute,
            fromRoute,
            async params => {
              if (params) {
      ...
      if (beforeEachCount == 0 && beforeIntercept == null && to.meta.isTabBar) {
        ...
        const { selfRoutes } = Router;
        const beforeEnter = Reflect.get(
          selfRoutes[`/${to.meta.pagePath}`],
          "beforeEnter"
        );
        if (beforeEnter) {
...
export const appMount = function(Router) {
  if (vueMount.length == 0) {
    return err(
      `检测到您未进行dom模型挂载操作，请调用api\r\n\r\n RouterMount(Vim: any, el: any): void`
    );
  }
  const { Vim, el } = vueMount[0];
  if (el == null) {
